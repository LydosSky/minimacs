...
;; Get that environment variables
(use-package exec-path-from-shell
  :ensure t
  :config
  ;; Load variables like PATH, etc
  (dolist (var '("PATH"))
...
(use-package markdown-mode
  ...
  :mode (("\\.md\\'" . markdown-mode) ; Automatically activate for .md files
         ("\\.markdown\\'" . markdown-mode)
         ("\\.mdown\\'" . markdown-mode)
         ("\\.mkd\\'" . markdown-mode)
         ("\\.mkdn\\'" . markdown-mode)
         ("\\.text\\'" . markdown-mode)
         ("\\.txt\\'" . markdown-mode)) ; Be careful with .txt, might activate for non-markdown
  :config
  ;; Example: Some common customizations
  (setq markdown-italic-markup "*"   ; Use * for italic instead of _
        markdown-bold-markup "**"    ; Use ** for bold instead of __
        markdown-hide-markup t       ; Hide markup characters after parsing
        markdown-fontify-code-blocks-natively t) ; Use built-in Emacs font-lock for code blocks

  ;; Example: Set up markdown-preview to use pandoc if you have it
  ;; (setq markdown-command "pandoc")
  ;; (setq markdown-arguments '("-f" "markdown+smart" "-t" "html5" "--mathjax"))
  )
